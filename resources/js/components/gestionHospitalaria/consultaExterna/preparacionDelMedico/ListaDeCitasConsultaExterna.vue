...
export default {
    ...
    mounted: function() {
        ...
        let nombreFormulario = this.$nombresFormulario.gestion_hospitalaria
            .admistracion_de_citas.citas.lista_consulta_externa
            .nombre_formulario;
                    ...
                    for (let i = 0; i < response.data.datos.length; i++) {
                        let objeto = {
                            ...
                            fecha_cita:
                                response.data.datos[i].estado_cita
                                    .ESTADOCITA_FECHA,
                            nombre_mostrar_paciente: that.$funcionesGlobales.toCapitalFirstAllWords(
                                response.data.datos[i].paciente
                                    .FULL_NAME_IDENTIFICATION
                            ),
                            nombre_mostrar_doctor: that.$funcionesGlobales.toCapitalFirstAllWords(
                                response.data.datos[i].estado_cita.doctor.user
                                    .FULL_NAME
                            ),
                            signo_vital:
                                response.data.datos[i].informacion_inicial !=
                                null
                                    ? "Registrado"
                                    : "Pendiente"
                        };
