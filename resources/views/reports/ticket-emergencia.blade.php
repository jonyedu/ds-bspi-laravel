...
<p>Se genero el ticket número {{$dato->numero_ticket}}, para su cita médica con fecha {{$dato->created_at_parseado}} desde las {{$dato->ESTADOCITA_HORA_INICIAL}} hasta {{$dato->ESTADOCITA_HORA_FINAL}} con el/la Dr(a). {{$dato->doctor->user->FULL_NAME}} para la especialidad {{$dato->especialidad->ESPECIALIDAD_NOM}}, por favor diríjase a la sala de espera para seguir con el proceso.</p>
